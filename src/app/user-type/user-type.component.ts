...
export class UserTypeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.getUserType(this.countrySelected, 'owner/', this.pageNo, '').subscribe(result => {
  ...
  userNext(utid:string){
    ...
    this.dataService.getUserType(this.countrySelected, utid, this.pageNo, this.searchString, this.dateSelected).subscribe(result => {
      if(result.length > 1){
        this.isLoading = false;
        let lastIndex = result.length - 1;
        this.totalCount = this.totCount[lastIndex].totlprop;
  ...
  applyFilterPlan(filterValue) {
    ...
    this.dataService.getUserType(this.countrySelected, this.selectedItem, this.pageNo, this.searchString, this.dateSelected).subscribe(result => {
      if(result.length > 1){ 
        result = this.totCount = result;
        let lastIndex = result.length - 1;
        this.totalCount = this.totCount[lastIndex].totlprop;
  ...
  changeCountry(countryCode) {
    ...
    this.dataService.getUserType(this.countrySelected, this.selectedItem, this.pageNo, '', this.dateSelected).subscribe(result => {
      if(result.length > 1){ 
        result = this.totCount = result;
        let lastIndex = result.length - 1;
        this.totalCount = this.totCount[lastIndex].totlprop;
  ...
  pageNext(filterValue){
    ...
    this.dataService.getUserType(this.countrySelected, this.selectedItem, this.pageNo, filterValue).subscribe(result => {
      if(result.length > 1){
        result = this.totCount = result;
        let lastIndex = result.length - 1;
        this.totalCount = this.totCount[lastIndex].totlprop; 
