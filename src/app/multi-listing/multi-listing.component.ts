...
export class MultiListingComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private dataService: DataService, public dialog: MatDialog, private httpService: HttpClient, private header: HeaderComponent, private datePipe: DatePipe) {
  ...
  ngOnInit() {
    ...
    this.dataService.getPlan(this.countrySelected, this.activeElement || 'all', this.pageNo,'', this.date).subscribe(result => {
  ...
  refreshData(){
    ...
    this.dateSelected = '';
    this.pageNo = 1;
    this.dataService.getPlan(this.countrySelected, this.activeElement || 'all', this.pageNo,'','').subscribe(result => {
      result = this.totCount = result;
      let lastIndex = result.length - 1;
      this.totalCount = this.totCount[lastIndex].totlprop;
      this.isLoading = false;
  ...
  planNext(pid:string){
    ...
    this.dataService.getPlan(this.countrySelected, pid, this.pageNo, this.searchString, this.dateSelected).subscribe(result => {
      result = this.totCount = result;
      if(result.length > 1){
        this.isLoading = false;
        let lastIndex = result.length - 1;
  ...
  applyFilterPlan(filterValue) {
    ...
    this.dataService.getPlan(this.countrySelected, this.activeElement || 'all', this.pageNo, filterValue, this.dateSelected).subscribe(result => {
      if(result.length > 1){ 
        result = this.totCount = result;
        let lastIndex = result.length - 1;
        this.totalCount = this.totCount[lastIndex].totlprop;
  ...
  applyDate(startdate) {
    ...
    if(this.dateSelected!=''){
      this.dataService.getPlan(this.countrySelected, this.activeElement || 'all', this.pageNo, this.searchString,this.dateSelected).subscribe(result => {
        if(result.length > 1){ 
          result = this.totCount = result;
          let lastIndex = result.length - 1;
          this.totalCount = this.totCount[lastIndex].totlprop;
  ...
  changeCountry(countryCode) {
    ...
    this.dataService.getPlan(countryCode, this.activeElement || 'all', this.pageNo, '', this.dateSelected).subscribe(result => {
      if(result.length > 1){ 
        let lastIndex = result.length - 1;
        result = this.totCount = result;
        this.totalCount = this.totCount[lastIndex].totlprop;
