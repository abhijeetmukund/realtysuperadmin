...
export class ResponseComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'lead_id', 'property_unique_id', 'property_id', 'owner','leads_status_id','name','email','city','mobileno','type','applied_date','lead_ip_address','reported_as_broker_status','leads_score','calls','lead_prefer_timings','lead_prefer_bhk','lead_prefer_location','lead_prefer_budget','lead_prefer_service_type','property_suggested','property_visited1','property_visited2','property_visited3','allocate_person','visited_date'];

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;

  constructor(private toastr: ToastrService, private dataService: DataService, public dialog: MatDialog, private httpService: HttpClient, private datePipe: DatePipe) {
    this.editLead = {
      "lead_id": "",
      "receiver_id": "",
      "user_id": "",
  ...
  uploadFiles () {
    ...
    this.httpService.post('https://www.realtymonk.com/websapi/leadmanagement/import', frmData).subscribe(
      data => {
  ...
  applyFilter(filterValue) {
    ...
    this.dataService.getResponse(this.countrySelected, this.pageNo, filterValue, this.dateSelected).subscribe(result => {
      if(result.length > 1){ 
        result = this.totCount = result;
        let lastIndex = result.length - 1;
        this.totalCount = this.totCount[lastIndex].ttlleads;
  ...
  applyDate(startdate) {
    ...
    if(this.dateSelected!=''){
      this.dataService.getResponse(this.countrySelected, this.pageNo, this.searchString, this.dateSelected).subscribe(result => {
        if(result.length > 1){
          result = this.totCount = result;
          let lastIndex = result.length - 1;
          this.totalCount = this.totCount[lastIndex].ttlleads;
  ...
  changeCountry(countryCode) {
    ...
    this.dataService.getResponse(countryCode, this.pageNo, '', this.dateSelected).subscribe(result => {
      if(result.length > 1){ 
        result = this.totCount = result;
        let lastIndex = result.length - 1;
        this.totalCount = this.totCount[lastIndex].ttlleads;
  ...
  getLeadId(event,leadId,inx){
    let indx = this.leadvalue.indexOf(leadId);
    if(event.checked){
      this.count=this.count+1;
      this.leadvalue.push(leadId);
      this.tableIndex.push(inx);
    }
    else{
      this.count=this.count-1;
      this.leadvalue.splice(indx,1);
    }
  }
  ...
  editLeads(){
    this.dataService.editLeads(this.editLead).subscribe(data => {
      ...
      this.leadvalue = [];
      this.count = 0;    
      $("#editSection_respons").hide();
      this.refreshData();
    });
  }

  openUserDetails(){
    this.dialog.open(UserdetailsComponent, { disableClose: true });
  }
  ...
  editRespPopup(event){
    if(this.count==1){
    $("#editSection_respons").show();
    let lead_id =this.leadvalue[0];
      if(lead_id!=undefined){

        this.dataService.getLeadId(lead_id).subscribe(data=>{
          this.editLead.lead_id=data[0].lead_id;
          this.editLead.receiver_id=data[0].receiver_id;
          this.editLead.user_id=data[0].user_id;
          this.editLead.name=data[0].name;
          this.editLead.email=data[0].email;
          this.editLead.mobileno=data[0].mobileno;
          this.editLead.applied_date=data[0].applied_date;
          this.editLead.calls=data[0].calls;
          this.editLead.lead_prefer_timings=data[0].lead_prefer_timings;
          this.editLead.lead_prefer_bhk=data[0].lead_prefer_bhk;
          this.editLead.lead_prefer_location=data[0].lead_prefer_location;
          this.editLead.lead_prefer_budget=data[0].lead_prefer_budget;
          this.editLead.lead_prefer_service_type=data[0].lead_prefer_service_type;
          this.editLead.property_visited1=data[0].property_visited1;
          this.editLead.property_visited2=data[0].property_visited2;
          this.editLead.property_visited3=data[0].property_visited3;
          this.editLead.allocate_person=data[0].allocate_person;
          this.editLead.visited_date=data[0].visited_date;
          this.editLead.visting_time=data[0].visting_time;
          this.editLead.follow_up1=data[0].follow_up1;
          this.editLead.follow_up2=data[0].follow_up2;
          this.editLead.follow_up3=data[0].follow_up3;
          this.editLead.status=data[0].status;
          this.editLead.portal_details =data[0].portal_details;
          this.editLead.allocationpersonemail=data[0].allocationpersonemail;
          this.editLead.allocationpersonphoneno=data[0].allocationpersonphoneno;
        });
      }
    }else if(this.count<1){
      this.toastr.warning('Select 1 record to edit',null,{
        positionClass: 'toast-center-center'
      });
    }
    else{
      this.toastr.warning('Editing multiple records are not allowed', 'Choose single record only',{
        positionClass: 'toast-center-center'
      });
    }
  }
  ...
  deleteLead(){
    let lead_id =this.leadvalue[0];
    if(lead_id!=undefined){
      this.dataService.deleteLead(this.leadvalue[0]).subscribe(data=>{
        this.leadvalue=[];
        this.totCount.splice(0, 1);
      });
    }
    this.refreshData();
  ...
  showDeletePopUp(){
    ...
    if(lead_id!=undefined){
      ...
      this.toastr.warning('Select atleast one record to delete',null,{
        positionClass: 'toast-center-center'
      });
    }
