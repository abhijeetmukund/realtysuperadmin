...
export class PropertyTypeComponent{  
  ...
  connect(): Observable<User[]> {
    return this.dataService.getProperty(this.countrySelected, 'residential', this.pageNo, '');
  }
  ...
  userNext(utid:string){
    ...
    this.dataService.getProperty(this.countrySelected, utid, this.pageNo, '').subscribe(result => {
      this.isLoading = false;
      if(!result){
        return ;
      }
  ...
  changeCountry(countryCode, filterValue) {    
    this.dataService.getProperty(countryCode, this.selectedItem, this.pageNo, filterValue).subscribe(result => {
      if(!result){
        return ;
      }
      this.dataSource= new MatTableDataSource(result);
