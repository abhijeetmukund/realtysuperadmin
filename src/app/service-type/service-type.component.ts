...
export class ServiceTypeComponent{
  ...
  connect(): Observable<User[]> {
    return this.dataService.getService(this.countrySelected, 'sale/', this.pageNo, '');
  }
  ...
  userNext(sid:string){
    ...
    this.dataService.getService(this.countrySelected, sid, this.pageNo, '').subscribe(result => {
      this.isLoading = false;
      if(!result){
        return ;
      }
  ...
  changeCountry(countryCode, filterValue) {    
    this.dataService.getService(countryCode, this.selectedItem, this.pageNo, filterValue).subscribe(result => {
      if(!result){
        return ;
      }
      this.dataSource= new MatTableDataSource(result);
